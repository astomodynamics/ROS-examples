...
class DetectObjectNode(Node): 
    def __init__(self):
        ...
        self.object_location_publisher = self.create_publisher(
            Point, "/object/object_location", 10)

        self._img_subscriber # Prevents unused variable warning.

        self._field_of_view = 62.2
    ...
    def _image_callback(self, CompressedImage):
        ...
        if len(contours) > 0:
            ...
            if radius > 20:
                ...
                object_angle = self._field_of_view * x / self._pixel_width - self._field_of_view/2
                object_angle = np.arctan2(np.sin(np.deg2rad(object_angle)), np.cos(np.deg2rad(object_angle)))
                object_location.x = object_angle
                object_location.y = 0.
                object_location.z = 0.
                self.object_location_publisher.publish(object_location)
            else:
                object_location.x = 9.9
                object_location.y = 0.
                object_location.z = 0.
                self.object_location_publisher.publish(object_location)
