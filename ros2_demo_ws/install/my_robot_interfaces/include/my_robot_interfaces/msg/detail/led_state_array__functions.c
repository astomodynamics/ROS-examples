  ...
  my_robot_interfaces__msg__LedStateArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    my_robot_interfaces__msg__LedStateArray * data =
      (my_robot_interfaces__msg__LedStateArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_robot_interfaces__msg__LedStateArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          my_robot_interfaces__msg__LedStateArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
