...
class TurtleSpawnerNode(Node):
    def __init__(self):
        ...
        self.elinimate_turtle_service_ = self.create_service(
            EliminateTurtle, "eliminate_turtle", self.callback_eliminate_turtle)

    def callback_eliminate_turtle(self, request, response):
    ...
    def callback_call_kill(self, future, turtle_name):
        try:
            future.result()
            for (ite, turtle) in enumerate(self.alive_turtles_):
                if turtle.name == turtle_name:
                    del self.alive_turtles_[ite]
                    self.publish_alive_turtles()
                    break
